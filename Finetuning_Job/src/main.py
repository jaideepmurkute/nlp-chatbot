...
"""
QLoRA:
Loads the pretrained base model with 4-bit NF4 quantized weights using bitsandbytes; 
weights are dequantized to FP16/BF16 only for compute.

Base model parameters are frozen and never updated; no FP32 master copy exists.

Prepares the quantized model for stable training (freezing, higher-precision norms).

Adds LoRA adapters, which are the only trainable parameters.

LoRA weights are trained in full precision with standard backpropagation and AdamW.

Forward pass computes base model output + LoRA update, enabling memory-efficient fine-tuning (QLoRA).
"""
    ...
    for step, batch in enumerate(progress_bar):
        ...
        if step % log_interval == 0:
            ...
            if cfg.get("use_wandb", False):
                wandb.log({"train/loss": current_loss, "epoch": 1})
...
def train(cfg):
    ...
    if cfg.get("use_wandb", False):
        wandb.init(
            project=cfg.get("wandb_project", "nlp-chatbot-finetune"),
    ...
    if cfg.get("use_lora", False):
        try:
            ...
            # Prepare the 4-bit model for training (Freezes layers, casts norms to fp32)
            if use_4bit:
            ...
            peft_config = LoraConfig(
                ...
                r=cfg.get(
                    "lora_r", 8
                ),  # Rank of the update matrices (The "capacity" of new info provided)
                lora_alpha=cfg.get(
                    "lora_alpha", 32
                ),  # Scaling factor (Like learning rate multiplier: Scale = alpha/r)
    ...
    train_idx, val_idx = train_test_split(
        indices, test_size=0.1, random_state=cfg["seed"], shuffle=True
    )
    ...
    total_steps = (
        len(train_loader) * cfg["epochs"] // cfg.get("gradient_accumulation_steps", 1)
    )
    ...
    for epoch in range(cfg["epochs"]):
        ...
        train_loss, train_perplexity = train_epoch(
            cfg, model, train_loader, optimizer, scheduler, device
        )
        ...
        print(
            f"Loss: Train: {training_log['train']['loss'][-1]} \t \
                Val: {training_log['val']['loss'][-1]}"
        )
        print(
            f"Perplexity: Train: {training_log['train']['perplexity'][-1]} \t \
        ...
        if val_loss < best_val_loss:
            ...
            print(
                f"New best model found (val_loss: {val_loss:.4f}). Saving to {best_model_path}..."
            )
            ...
            if epoch - last_best_epoch >= cfg["early_stopping_patience"]:
                print(
                    f"No improvement in validation loss for {cfg['early_stopping_patience']} epochs."
                )
