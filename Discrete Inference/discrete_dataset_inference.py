...
class DiscreteInference:
    ...
    def __save_responses(self, dataset_df, responses):
        assert os.path.exists(self.config['output_dir']), \
            f"Output directory not found - {self.config['output_dir']}"
            
        op_df = pd.DataFrame({'input': dataset_df['input'], 
                              'response': responses})
    ...
    def __load_dataset(self):
        assert os.path.exists(self.config['data_dir']), \
            f"Data directory not found - {self.config['data_dir']}"
    ...
    def __encode_dataset(self, dataset, tokenizer):
        ...
        tokenized_data = dataset['input'].apply(tokenize_function)
    ...
    def __generate_response(self, encoded_data, model, tokenizer):
        ...
        responses = tokenizer.batch_decode(op_cls.tolist(), 
